...
check('offline precache', () => {const serviceWorker=read('service-worker.js');[...scripts.filter(file=>file.startsWith('assets/js/')),...lessons,...worksheets,...guides].forEach(file=>assert.ok(serviceWorker.includes(`./${file}`),file));assert.match(serviceWorker,/arshavin-grade4-v22/);});
check('CIT-04 safeguards and completion', () => {const material=read(lessons[20])+read(guides[19])+read(worksheets[20]);const script=read('assets/js/peaceful-dialogue-lesson.js');['fact','feeling','need','request','CALM'].forEach(term=>assert.match(material,new RegExp(term,'i')));assert.match(material,/สถานการณ์สมมติ|fictional/i);assert.match(material,/ข่มขู่|threat/i);assert.match(material,/ผู้ใหญ่ที่ไว้ใจได้|trusted adult/i);assert.match(material,/ไม่บังคับ.*ขอโทษ|not.*force.*apolog/i);['dialogueComplete','needsComplete','quizComplete'].forEach(key=>assert.match(script,new RegExp(key)));assert.doesNotMatch(material,/draggable="true"/);});
