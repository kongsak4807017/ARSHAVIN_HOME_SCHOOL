...
const scripts = [
  ...
  'assets/js/ai-claims-lesson.js','assets/js/privacy-consent-lesson.js','assets/js/data-bias-lesson.js','assets/js/pm25-lesson.js','assets/js/water-cycle-lesson.js',
...
const keys = [
  ...
  'arshavin.ai.claims.v1','arshavin.ai.privacy.v1','arshavin.ai.databias.v1','arshavin.environment.pm25.v1','arshavin.environment.water.v1',
...
check('shell exposes all lessons and keys', () => {const shell=read('assets/js/learning-shell.js');['HB-01','HB-02','HB-03','HB-04','AI-01','AI-02','AI-03','ENV-01','ENV-02','MAKER-01','MAKER-02','CIT-01','CIT-02'].forEach(id=>assert.ok(shell.includes(id),id));keys.forEach(key=>assert.ok(shell.includes(key),key));});
...
check('offline precache covers all runtime assets', () => {const sw=read('service-worker.js');['assets/js/learning-shell.js',...scripts.filter(file=>file.startsWith('assets/js/')&&file!=='assets/js/learning-shell.js'),...lessons,...worksheets,...guides].forEach(file=>assert.ok(sw.includes(`./${file}`),file));assert.match(sw,/arshavin-grade4-v14/);});
...
check('environment safety labels',()=>{assert.match(read(lessons[7]),/ไม่ใช่ประกาศคุณภาพอากาศปัจจุบัน/);assert.match(read(lessons[8]),/สถานการณ์จำลอง/);assert.match(read(lessons[8])+read(guides[7]),/(ไม่|ห้าม)ชิมน้ำ/);});
check('maker safety and native controls',()=>{const maker=read(lessons[10])+read(guides[9]);assert.match(maker,/ห้ามยกคน/);assert.match(maker,/จุดหนีบ/);assert.match(maker,/แบบจำลองอุดมคติ/);assert.doesNotMatch(maker,/draggable="true"/);});
check('citizenship consent and inclusive decision safeguards',()=>{assert.match(read(lessons[11]),/การยินยอม/);assert.match(read(lessons[11]),/ผู้ใหญ่ที่ไว้ใจได้/);const cit=read(lessons[12])+read(guides[11]);assert.match(cit,/VOICE/);assert.match(cit,/สถานการณ์จำลอง/);assert.match(cit,/ไม่ใช้การลงคะแนน/);});
