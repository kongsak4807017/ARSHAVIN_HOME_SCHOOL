...
check('integration',()=>{const index=read('index.html'),shell=read('assets/js/learning-shell.js'),sw=read('service-worker.js');[lesson,script,worksheet,guide].forEach(file=>assert.ok(sw.includes(`./${file}`),file));assert.ok(index.includes(lesson));assert.match(index,/arshavin\.citizenship\.budget\.v1/);assert.match(shell,/CIT-07/);assert.match(shell,/arshavin\.citizenship\.budget\.v1/);assert.match(sw,/arshavin-grade4-v\d+/)});
