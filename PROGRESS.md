...
## Completion counters
- Interactive bilingual lessons: **38 / 52**
- A4 student worksheets: **76 / 104 minimum**
- Teacher guides/rubrics: **37 complete guides**
...
## 2026-07-21 — AI-08 Generative AI, Provenance and Verification Before Use
- [x] Complete bilingual lesson defining generative AI, prompt, output, provenance and evidence.
- [x] Keyboard-native local-only prompt/output comparison with three fictional use cases and corrective bilingual feedback.
- [x] PROVE workflow: Pause, Read the claim, Observe provenance, Verify with evidence, Explain and escalate.
- [x] Explicit boundary that provenance records source/history but does not independently guarantee truth.
- [x] No child image/voice upload, real-person impersonation, deepfake creation, learner profiling or outbound lesson API.
- [x] Guarded local-only completion under `arshavin.ai.generative.v1` using `comparisonComplete`, `verificationComplete` and `quizComplete`.
...
- [x] Homepage, thirty-eight-lesson shell, reset key, service-worker v39, focused AI-08 checks and complete regression integration implemented on the feature branch.

## Acceptance status
**ACCEPTED WITH CONDITIONS** — thirty-eight lessons, seventy-six printable A4 sheets and thirty-seven teacher guides are integrated at source level. Exact final-head CI is required before merge. Browser/device, assistive-technology, physical A4 print, GitHub Pages and offline-reload verification remain required.

## Next action
Build **ENV-08 น้ำท่วม ภัยแล้ง และชุมชนยืดหยุ่น / Floods, Droughts and Community Resilience** with a fictional keyboard-native risk-and-response model, Chiang Rai-appropriate preparedness decisions, no household vulnerability or precise-location collection, two A4 worksheets, teacher guide/rubric, offline integration and focused static regression coverage.